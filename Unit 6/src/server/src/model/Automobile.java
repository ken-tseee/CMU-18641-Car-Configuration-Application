...
public class Automobile implements Serializable {
	 ...
	 */
	public Automobile(String model, float basePrice) {
		this.model = model;
	...
	public ArrayList<String> getOptionList(String setName) {
		...
		for(int i=0;i<optSet.size(); ++i) {
			if(optSet.get(i).getName().equals(setName)) {
				list = optSet.get(i).getOptionList();
			}
		}
		return list;
	}
	...
	public float getOptionChoicePrice(String setName) {
		for(int i=0; i<optSet.size(); i++) {
			if(optSet.get(i).getName().equals(setName)) {
				return optSet.get(i).getOptionChoicePrice();
			}
		}
		return 0;
	}
	...
	public ArrayList<String> getOptionList(int i) {
		return optSet.get(i).getOptionList();
	}

	public ArrayList<Float> getOptionPriceList(int i) {
		return optSet.get(i).getOptionPriceList();
	}
}
